...
<script type="text/javascript">
	...
	function minuFun(articleId) {
		...
		if (num > 1) {
			...
			//发送请求更新购物车中购买的数量 
			window.location = "${pageContext.request.contextPath}/shopCar/updateShopcar.action?id="
					+ articleId + "&number=" + obj.value;
	...
	function checkboxFun(info) {
		...
		//alert("总个数："+boxes.length+" 选中的个数："+num);
...
<body>
	...
	<div class="container">
							...
							<c:forEach items="${shopCars}" var="shopCar">
								<tr>
									<td style="vertical-align: middle;"><input type="checkbox"
										...
										onclick="checkboxFun(this.value)" /> <span id="total_1"
									...
									<td>价格:<span
								        ...
								        折后价:<span class="discountPrice"><fmt:formatNumber
													value="${shopCar.article.discountPrice}" pattern="0.00"></fmt:formatNumber></span>
										</c:if>
									</td>
									<td><span class="glyphicon glyphicon-minus"
										...
										onblur="validNum(this,'${shopCar.article.id}')" /> <span
										class="glyphicon glyphicon-plus"
										onclick="addFun('${shopCar.article.id}')"></span></td>
									<td><fmt:formatNumber
											value="${shopCar.article.discountPrice* shopCar.buynum}"
											pattern="0.00"></fmt:formatNumber></td>
									<td><a
										href="${pageContext.request.contextPath}/shopCar/deleteShopCar.action?id=${shopCar.article.id}">删除</a>
										<a>收藏</a></td>
								</tr>
							</c:forEach>
		...
		<!--/row-->
		<form
			action="${pageContext.request.contextPath}/order/orderSubmit.action"
			method="post" id="form">
			<!-- 通过隐藏输入框 将订单相关信息传递至后台 -->
			<input type="text" name="orderInfo" id="orderInfo" />
			<div align="right">
				购物车商品总金额:<span id="totalMoney" style="color: red;"><fmt:formatNumber

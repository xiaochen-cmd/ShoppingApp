		...
		<div class="row row-offcanvas row-offcanvas-right">
			...
			<div class="col-xs-12 col-sm-9">
				...
				<div class="alert alert-info" role="alert"
						...
						<c:forEach items="${secondArticleTypes}" var="secondArticleType">
							<c:choose>
								<c:when test="${secondArticleType.code eq secondCode}">
									<a
										href="${pageContext.request.contextPath}/article/index.do?typeCode=${secondArticleType.code}"
					...
					<div>
						<form action="${pageContext.request.contextPath}/article/index.do"
							method="get">
							<!-- 如果当前选择了商品的类型，仅在该类型下面进行搜索 -->
							<input type="hidden" name="typeCode" value="${typeCode}" /> <input
								name="keyword" value="${keyword}"
								style="width: 476px; height: 38px; border-radius: 100px; background: #FCFCFC; outline: 0 none; border: 1px solid #e2e2e2;" />
							<button type="submit"
								style="width: 126px; height: 38px; border-radius: 100px; background: #FCFCFC; outline: 0 none; border: 1px solid #e2e2e2; color: #333">搜索</button>
						</form>
					</div>
				</div>
					...
					<c:forEach items="${articles}" var="article">
						<div class="col-xs-6 col-lg-4">
							<span class="thumbnail"> <a
								...
								<p>
									<span class="price">${article.price}</span>&nbsp;<font
										color='red'>折后价:<%-- <fmt:formatNumber value="${article.discountPrice}" pattern="0.00"></fmt:formatNumber>--%></font>
				...
				<div class="row">
					<nav style="text-align: center;">
						<ul class="pagination">
							<longIt:pager
								submitUrl="${pageContext.request.contextPath}/article/index"
								pageModel="${pageModel}"></longIt:pager>
